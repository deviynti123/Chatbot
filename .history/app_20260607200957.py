...
def get_response(user_input, user="Teman", history=[]):
    ...
    # ── Pesan pendek / referensial → langsung Gemini pakai konteks history ──
    KATA_REFERENSIAL = [
    ...
    is_referensial = any(user_input.lower().strip().startswith(k) for k in KATA_REFERENSIAL)

    if history and (is_short or is_referensial):
        conn.close()
...
def chat_api():
    ...
    # Ambil 6 pesan terakhir sebagai history konteks (3 pasang tanya-jawab)
    cursor.execute(
        "SELECT sender, message FROM messages WHERE room_id=? ORDER BY id DESC LIMIT 6",
        (room_id,)
    )
